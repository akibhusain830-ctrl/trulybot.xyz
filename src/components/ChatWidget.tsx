...
export default function ChatWidget({ onClose }: { onClose?: () => void }) {
      ...
      if (savedHistory) {
        ...
        if (oldSavedHistory) {
          try {
            ...
            if (Array.isArray(parsedHistory) && parsedHistory.length > 0) {
              console.log("Chat history loaded from legacy key", {
                count: parsedHistory.length,
              });
  ...
  useEffect(() => {
    if (widgetConfig && botId && botId !== "demo") {
      ...
      // Apply accent color
      if (
        widgetConfig.accent_color &&
        widgetConfig.accent_color !== "#00D4FF"
      ) {
        customCSS += `
  ...
  useEffect(() => {
    ...
    if (savedHistory && savedHistory.length > 0) {
      ...
      console.log("Successfully restored chat history", {
        messageCount: savedHistory.length,
      });
      logger.info("Restored chat history", {
        messageCount: savedHistory.length,
      });
    } else {
      ...
      if (!seen) {
        ...
        const demoWelcome =
          botId === "demo"
            ? "👋 Hi! I'm TrulyBot's AI Assistant - this is exactly what YOUR customers will experience on your website!\n\n💡 **Live Demo in Action:** This professional, instant AI support will reduce your support tickets by 70% and capture 5X more leads!\n\nTry asking me about:\n• Pricing plans and free trial\n• WooCommerce integration\n• Features and setup process\n• How to get started\n\nYour customers will get this same instant, intelligent experience 24/7! ⚡"
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        const errorPayload = await res
          .json()
          .catch(() => ({ error: "Server error with no body" }));
      ...
      console.log(
        "CONTAINS __BUTTONS__:",
        fullResponse.includes("__BUTTONS__"),
      );
      ...
      if (buttonStart !== -1) {
        ...
        const buttonData = fullResponse.substring(
          buttonStart + buttonMarker.length,
        );
        ...
        try {
          ...
          logger.info("Successfully parsed buttons:", {
        ...
        const buttonData = fullResponse.substring(
          buttonLegacyStart + buttonLegacyMarker.length,
        );

        console.log("LEGACY BUTTON PARSING:", { finalText, buttonData });
        logger.info("Processing legacy button data:", {
          finalText,
          buttonData,
        });
        ...
        try {
          ...
          logger.info("Successfully parsed legacy buttons:", {
            buttons: parsedButtons,
          });
        } catch (e) {
          console.error("LEGACY BUTTON PARSE ERROR:", e, buttonData);
          logger.error("Failed to parse legacy buttons:", {
            error: e,
            buttonData,
          });
        }
      ...
      if (parsedButtons) {
        ...
        console.log(
          "- Length:",
          Array.isArray(parsedButtons) ? parsedButtons.length : "N/A",
        );
        console.log(
          "- Sample Item:",
    ...
    <div className="anemo-card-widget">
      <div className="anemo-card-header">
        ...
        <div className="anemo-card-header-actions">
          ...
          {onClose && (
            <button
              className="anemo-card-header-close"
              onClick={onClose}
        ...
        <div className="anemo-messages" ref={listRef}>
          {messages.map((m) => (
            <div key={m.id} className={`anemo-msg ${m.role}`}>
              <div
                className={`anemo-bubble ${m.role} ${m.error ? "error" : ""}`}
              >
                <div className="bubble-text">{m.text}</div>
                {m.sources && m.sources.length > 0 && (
                  <div className="anemo-sources">
                    <div className="sources-label">
                      Answer sourced from your documents:
                    </div>
                ...
                {m.usedDocs === false &&
                  ...
                  botId !== "demo" && (
                    <div className="anemo-fallback-note">
                      I&apos;d be happy to get you detailed information about
                      this. Could you share your email or phone number so I can
                      have our team follow up with you directly?
                    </div>
                  )}
                {/* Check if text contains a button marker (for debugging only) */}
                {m.text &&
                (m.text.includes("__BUTTONS__") ||
                  m.text.includes("___BUTTONS___")) ? (
                  <div className="anemo-error-note">
                    Error: Button data is showing as text. Please contact
                    support.
                  </div>
                ) : null}
                      ...
                      {m.buttons.map((button, btnIndex) => {
                        ...
                        return (
                          <a
                            ...
                            href={buttonUrl}
                            target={
                              buttonUrl.startsWith("http") ? "_blank" : "_self"
                            }
                            rel={
                ...
                {m.followUpQuestions &&
                  ...
                  m.followUpQuestions.length > 0 && (
                    <div className="anemo-follow-up-questions">
                ...
                {m.suggestedTopics &&
                  Array.isArray(m.suggestedTopics) &&
                  m.suggestedTopics.length > 0 && (
                    <div className="anemo-suggested-topics">
                      <div className="topics-label">🔍 Also explore:</div>
                      <div className="topics-chips">
                        {m.suggestedTopics.slice(0, 2).map((topic, idx) => (
                          <button
                            key={idx}
                            className="topic-chip"
                            onClick={() =>
                              handleSubmit(`Tell me more about ${topic}`)
                            }
                          >
                            {topic}
                          </button>
      ...
      <style jsx global>{`
        ...
        .anemo-card-header-actions {
          display: flex;
          align-items: center;
          gap: 2px; /* Reduced spacing between buttons */
        }
        .anemo-clear-history-btn {
        ...
        .anemo-clear-history-btn:hover {
          background: #292f35;
          color: #fff;
        }
        .anemo-card-header-close {
          background: transparent;
          border: none;
          color: #a5aebf;
          font-size: 1.6rem;
        ...
        .anemo-messages::-webkit-scrollbar {
          display: none;
        }
        .anemo-msg {
          display: flex;
        ...
        .anemo-sources .sources-chips {
          display: flex;
          flex-wrap: wrap;
          gap: 6px;
        }
        .anemo-sources .source-chip {
          background: #282c34;
        ...
        .anemo-sources .source-chip:hover {
          background: #2563eb22;
          border-color: #2563ebaa;
        }
        .anemo-sources .source-dot {
          width: 6px;
          height: 6px;
          border-radius: 50%;
          background: #2563eb;
        ...
        .anemo-button.primary {
          background: #2563eb;
          color: #fff;
          border: 1px solid #2563eb;
        }
        .anemo-button.primary:hover {
          background: #1d4ed8;
          border-color: #1d4ed8;
          transform: translateY(-1px);
        }
        .anemo-button.secondary {
          background: #374151;
          color: #d1d5db;
