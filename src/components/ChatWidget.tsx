...
const STORAGE_KEY = 'anemo_conv_ui_final_v4';
const INTRO_KEY = 'anemo_seen_intro_ui_final_v4';
const SUGGESTIONS = [
...
  const submit = useCallback((raw?: string) => {
    ...
    callApi(prompt);
  }, [input, loading, suggestions, callApi, push]);
...
      <style>{`
      ...
      * {
      ...
      .anemo-chat-root {
        ...
        height: 100vh; /* Use 100dvh in a real app for best mobile results */
        width: 100%;
        margin: 0;
        padding: 0;
        overflow: hidden;
      ...
      .body::-webkit-scrollbar-thumb { background-color: rgba(125, 125, 125, 0.2); border-radius: 20px; border: 2px solid var(--card-bg); background-clip: content-box; }
      ...
      .bubble .text { font-size: 0.9rem; line-height: 1.6; white-space: pre-wrap; word-wrap: break-word; }
      .bubble .meta { font-size: 0.7rem; color: var(--text-secondary); text-align: right; }
      .bubble.user .meta { color: rgba(255,255,255,0.7); }
      
      .bubble.typing .dots { display: flex; gap: 4px; align-items: center; padding: 4px 0; }
      ...
      .sugs { display: flex; gap: 8px; padding: 0 20px 12px; overflow-x: auto; flex-shrink: 0; }
      .chip { flex-shrink: 0; background: var(--bot-bubble-bg); border: 1px solid var(--border-color); color: var(--text-secondary); padding: 8px 16px; border-radius: 20px; font-size: 0.8rem; cursor: pointer; transition: background-color 0.2s; }
      .chip:hover { background: #30363d; color: var(--text-primary); }
      ...
      .composer textarea {
        flex-grow: 1; background: var(--bot-bubble-bg); border: none;
        border-radius: 20px; padding: 10px 16px; outline: none; resize: none; color: var(--text-primary);
        font-size: 0.95rem; font-family: inherit; line-height: 1.5; max-height: 100px;
      }
      .composer textarea:focus { box-shadow: 0 0 0 2px var(--accent-color); }
      .composer .send {
        background: var(--accent-color); color: white; border-radius: 50%; width: 36px; height: 36px;
      ...
      button.ghost { color: var(--text-secondary); width: 32px; height: 32px; display:flex; align-items:center; justify-content:center; border-radius: 50%; transition: background-color 0.2s; background: none; border: none; cursor: pointer; }
