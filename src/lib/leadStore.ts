...
export async function persistLeadIfAny(params: PersistLeadParams): Promise<{ created: boolean; id?: string }> {
  if (!params.email && !params.intentKeywords.length && !params.intentPrompt) {
  ...
  if (!name || !company) {
    const derived = deriveNameCompanyFromConversation(params.conversation || []);
    ...
    if (existing) {
      const { error: updErr } = await adminClient
        ...
        .update({
          ...
          name,
          company,
          meta: { intentPrompt: params.intentPrompt }
        })
        .eq('id', existing.id);
  ...
  const status = params.email ? 'new' : 'incomplete';
...
function truncateConversation(conv: Array<{ role: string; text: string }>, maxChars: number) {
  let total = 0;
...
function deriveNameCompanyFromConversation(conv: Array<{ role: string; text: string }>) {
  const userTexts = conv.filter(c => c.role === 'user').map(c => c.text).join('\n');
  let name: string | undefined;
  let company: string | undefined;

  const nameMatch = userTexts.match(/\b(?:my\s+name\s+is|i'm|i am)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+){0,2})/i);
  if (nameMatch) name = nameMatch[1].trim();

  const companyMatch = userTexts.match(/\b(?:at|from)\s+([A-Z][A-Za-z0-9&\-\s]{2,40})(?:[.,\n]|$)/);
  if (companyMatch) {
    const raw = companyMatch[1].trim();
    if (!/\b(pricing|price|plan|support|billing|cost|help)\b/i.test(raw)) company = raw;
  }
...
export function extractIntentKeywords(text: string): string[] {
  if (!text) return [];
  const KW = [
    'pricing','price','plan','plans','buy','purchase','trial','subscribe',
    'integration','support','cost','charge','billing','quote','demo'
  ];
