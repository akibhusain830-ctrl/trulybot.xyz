-- ================================================================
-- ULTRA-SAFE TRULYBOT DATABASE SETUP
-- Guaranteed to work with ANY Supabase project (fresh or existing)
...
-- ================================================================
-- 1. ULTRA-SAFE CLEANUP (NO ERRORS POSSIBLE)
-- ================================================================

-- Safe cleanup function that never fails
DO $$
BEGIN
    -- Drop triggers safely
    PERFORM 1 FROM information_schema.triggers WHERE trigger_name = 'on_auth_user_created' AND event_object_schema = 'auth';
    IF FOUND THEN
        DROP TRIGGER on_auth_user_created ON auth.users CASCADE;
    END IF;
    
    -- Drop other triggers if they exist
    ...
    -- Drop tables safely (CASCADE removes all dependencies including policies)
...
CREATE INDEX usage_counters_created_at_idx ON public.usage_counters(created_at);
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
        user_id := NULL;
        email := 'No auth.users table found';
        action := 'skipped';
        RETURN NEXT;
        RETURN;
    END IF;
...
-- ================================================================
-- 12. CREATE MISSING PROFILES AND FINAL VERIFICATION
-- ================================================================

-- Create any missing profiles (safe - won't fail if no users exist)
...
BEGIN
    ...
    -- Report results
    RAISE NOTICE '=== TRULYBOT DATABASE SETUP COMPLETE ===';
    ...
    IF table_count = 4 AND function_count = 3 AND trigger_count = 1 THEN
        RAISE NOTICE '🎉 SUCCESS! Your TrulyBot database is ready!';
        RAISE NOTICE '✅ All components created successfully';
        RAISE NOTICE '🚀 You can now test your authentication system';
    ELSE
        RAISE WARNING '⚠️  Some components missing - check error messages above';
    END IF;
END $$;
...
UNION ALL
SELECT 
    'Tables: ' || COUNT(*) || ' created' as status,
    NOW() as completed_at
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('workspaces', 'profiles', 'usage_counters', 'leads');